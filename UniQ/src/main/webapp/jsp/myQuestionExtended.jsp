...
<body>
	...
	<section>
		...
		while(rs.next()){
		%>
		<input type="button" onclick="location.href='intent://구글 폼 결과 화면url#Intent;scheme=http;package=com.android.chrome;end'" value="<%=rs.getString("range")%> <%=rs.getString("title")%> <%=rs.getString("deadline")%>">
		<%
		str += "</td></tr>";
		}
		str += "</table>";
		out.print(str);
		%>
		<input type="button"  value = "편집" onClick="location.href='editMyQuestion.jsp'">
		<%
