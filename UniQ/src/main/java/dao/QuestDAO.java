...
public class QuestDAO {
	...
	public ArrayList<QuestObj> getListRegist(String id) throws NamingException, SQLException{
		Connection conn= ConnectionPool.get();
		PreparedStatement stmt= null;
		ResultSet rs= null;
		try{
	...
	public QuestObj getListJoin(int no) throws NamingException, SQLException {
		Connection conn= ConnectionPool.get();
		PreparedStatement stmt= null;
		ResultSet rs= null;
		
		try{
			String sql= "SELECT * FROM quest WHERE qno = ? ";
			stmt= conn.prepareStatement(sql);
			stmt.setInt(1, no);
			rs= stmt.executeQuery(); 
	...
	public boolean delete(int qno) throws NamingException, SQLException{
		Connection conn= ConnectionPool.get();
		PreparedStatement stmt= null;
		try{
			String sql= "DELETE FROM joinquest WHERE jno = ?";
			stmt= conn.prepareStatement(sql);
			stmt.setInt(1, qno);
			stmt.executeUpdate();
			stmt.close();
			
			sql= "DELETE FROM quest WHERE qno = ?";
			stmt= conn.prepareStatement(sql);
			stmt.setInt(1, qno);
